...
\subsection{Hardware}
The main hardware used for this project is a \textit{Tobii EyeTracker 4C}, a USB eye tracker using an infrared camera to track the gaze position and translate it to the screen. This system is relatively low cost and works well enough for many usages as unlike more expensive system, the point of the \textit{4C} isn't to properly track every single movments but more to determine the different area gazed upon. 
And for our porject we will use it in coordination with a standard keyboard.
...
The above example of code a button to be activated by the eye tracker. When it is activated, it calls the function \textit{activation\_function}.
